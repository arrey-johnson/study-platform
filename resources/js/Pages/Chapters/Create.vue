...
const form = useForm({
    title: '',
    content: '',
    order: props.module?.chapters?.length ? props.module.chapters.length + 1 : 1,
    pdf_notes: null,
    is_active: true
});

const submit = () => {
    form.post(route('modules.chapters.store', props.module.id), {
        forceFormData: true
    });
};
    ...
    <AuthenticatedLayout>
        <template #header>
            <h2 class="text-xl font-semibold leading-tight text-gray-800">
                Create Chapter for {{ module.title }}
            </h2>
        </template>
